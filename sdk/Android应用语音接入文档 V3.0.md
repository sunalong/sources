...
函数原型：
    public native boolean startRecordVoice(boolean needConvertWord,boolean isPersis, float scaleLevel);
```
参数说明：
...
```java
主线程中调用，进入房间之后调用才有效
	NativeVoiceEngine.getInstance().setVoiceChangeParm(result,reverbLevel);
函数原型：
	public native int setVoiceChangeParm(int pitch, int reverbLevel);
```
参数说明：
